...
<script src="cav.js"></script>
    ...
    <form  action="admin_book_jiechu.php?tsid=<?php echo $bookid; ?>" method="POST" style="position:absolute;margin-left:50%;margin-top:10%;transform:translate(-50%,0)">
        <div id="query">
            <label ><input  name="borrower" type="text" placeholder="请输入借阅人读者证号" style="height:45px;width:350px;font-size: 16px;text-align: center;outline: none;" class="inputLabel">
            <input type="submit" value="借阅" class="btn btn-default" style="height:45px;width:70px;border-radius:20px;outline:none;border:none;font-size: 15px;">
        </div>
    </form>
...
<?php
$sql="select book_id,name,author,publish,ISBN,language,price,pubdate,book_info.class_id,class_name,pressmark from book_info,class_info where book_info.class_id=class_info.class_id and book_info.book_id='{$bookid}'";
